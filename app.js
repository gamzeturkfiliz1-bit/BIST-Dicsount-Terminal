...
function showCustomConfirm(message, callbackConfirm, callbackCancel) {
        ...
        <div class="custom-confirm-card">
            <div class="custom-confirm-inner">
                ...
                <div class="custom-confirm-buttons">
                    ...
                    <button class="custom-confirm-btn confirm" id="confirmConfirmBtn">Evet, Sil</button>
                </div>
            </div>
        </div>
    `;
...
function updateDataStatus(state) {
    ...
    if (state === 'loading') {
        ...
        if (dataStatusDot) {
            dataStatusDot.style.backgroundColor = '#FFD600';
            dataStatusDot.style.boxShadow = '0 0 8px #FFD600';
        }
        if (dataStatusText) {
            dataStatusText.textContent = 'Lokal Veri';
            dataStatusText.style.color = '#FFD600';
        }
        if (infoText) {
            infoText.innerHTML = 'Lokal Veritabanı<br>Baseline / Kişisel Veriler Aktif';
            if (infoIcon) infoIcon.textContent = '💾';
        }
        if (banner && text) {
            banner.style.display = 'flex';
            banner.className = 'notification welcome';
            banner.style.backgroundColor = 'rgba(255, 214, 0, 0.15)';
            banner.style.border = '1px solid rgba(255, 214, 0, 0.3)';
            text.style.color = '#FFD600';
            text.textContent = '🟡 Lokal Veri Aktif — Canlı veriler alınamadı (offline/gecikme), yerel değerler kullanılıyor.';
        }
    }
}
...
async function triggerBackgroundFetch(sectorKey) {
    ...
    try {
        ...
        showToast('Bağlantı gecikmeli: Lokal veriler kullanılıyor.', 'info');
...
function mergeAndRenderSectors() {
    ...
    firebasePrivateSectors.forEach(cSec => {
        SECTORS[cSec.key] = { ...cSec.sectorData, isPrivateCloud: true };
        Object.keys(cSec.stocksData).forEach(ticker => {
            if (!ALL_STOCKS[ticker]) {
                ALL_STOCKS[ticker] = cSec.stocksData[ticker];
            }
    ...
    firebaseSharedSectors.forEach(cSec => {
        SECTORS[cSec.key] = { ...cSec.sectorData, isShared: true };
        Object.keys(cSec.stocksData).forEach(ticker => {
            if (!ALL_STOCKS[ticker]) {
                ALL_STOCKS[ticker] = cSec.stocksData[ticker];
            }
    ...
    Object.values(SECTORS).forEach(sec => {
        ...
        if (sec.key.startsWith('custom_')) {
            ...
            delIcon.addEventListener('mouseenter', () => {
                delIcon.style.background = 'var(--candy-red)';
                delIcon.style.color = '#ffffff';
                delIcon.style.transform = 'scale(1.1)';
            });
            delIcon.addEventListener('mouseleave', () => {
                delIcon.style.background = 'rgba(12, 14, 21, 0.7)';
                delIcon.style.color = 'var(--candy-red)';
                delIcon.style.transform = 'scale(1)';
            });
...
function showAddSectorModal() {
    ...
    modal.innerHTML = `
        <div style="position:relative; width:650px; max-width:95vw; background:var(--bg-1); border:1px solid var(--border); border-radius:var(--radius-lg); overflow:hidden; box-shadow:0 20px 40px rgba(0,0,0,0.6); display:flex; flex-direction:column; animation: modalSlide 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
            <div style="padding:16px 20px; border-bottom:1px solid var(--border); display:flex; align-items:center; justify-content:space-between;">
                <div>
                    <h3 style="font-family:var(--font-m); font-size:0.9rem; font-weight:700; color:var(--blue); letter-spacing:1px; margin:0;">➕ YENİ SEKTÖR TANIMLA</h3>
                    <p style="font-size:0.65rem; color:var(--text-300); margin:4px 0 0 0;">Sektör ortalamalarını ve hisse değerlerini belirleyin</p>
                </div>
            ...
            <div style="padding:20px; overflow-y:auto; max-height:70vh;">
                <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:12px; margin-bottom:20px;">
                    <div>
                        <label style="display:block; font-family:var(--font-m); font-size:0.6rem; font-weight:600; color:var(--text-300); letter-spacing:1px; text-transform:uppercase; margin-bottom:5px;">Sektör Adı</label>
                        <input type="text" id="modalSecName" class="sb-select" placeholder="Örn: Enerji" style="width:100%; box-sizing:border-box;" autocomplete="off">
                    </div>
                    <div>
                        <label style="display:block; font-family:var(--font-m); font-size:0.6rem; font-weight:600; color:var(--text-300); letter-spacing:1px; text-transform:uppercase; margin-bottom:5px;">Hisse 1 Kodu</label>
                        <input type="text" id="modalT1" class="sb-select" placeholder="Örn: EUPWR" style="width:100%; text-transform:uppercase; box-sizing:border-box;" autocomplete="off">
                    </div>
                    <div>
                        <label style="display:block; font-family:var(--font-m); font-size:0.6rem; font-weight:600; color:var(--text-300); letter-spacing:1px; text-transform:uppercase; margin-bottom:5px;">Hisse 2 Kodu</label>
                        <input type="text" id="modalT2" class="sb-select" placeholder="Örn: ALFAS" style="width:100%; text-transform:uppercase; box-sizing:border-box;" autocomplete="off">
                    </div>
                </div>
                
                <div style="border:1px solid var(--border); border-radius:var(--radius); overflow:hidden; background:var(--bg-0);">
                    <table style="width:100%; border-collapse:collapse; font-family:var(--font-m); font-size:0.75rem;">
                        <thead>
                            <tr style="background:var(--bg-2); border-bottom:1px solid var(--border);">
                                <th style="padding:10px; text-align:left; color:var(--text-300); font-weight:600;">Metrik</th>
                                <th style="padding:10px; text-align:center; color:var(--cyan); font-weight:600;">Sektör Ortalaması</th>
                                <th style="padding:10px; text-align:center; color:var(--t1); font-weight:600;" id="modalThT1">Hisse 1</th>
                                <th style="padding:10px; text-align:center; color:var(--t2); font-weight:600;" id="modalThT2">Hisse 2</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">F/K (P/E)</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mSecPe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="10.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1Pe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="8.50"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2Pe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="12.00"></td>
                            </tr>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">PD/DD (P/B)</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mSecPb" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="2.50"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1Pb" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="2.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2Pb" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="3.10"></td>
                            </tr>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">FD/FAVÖK (EV/EBITDA)</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mSecEvEbitda" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="8.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1EvEbitda" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="6.80"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2EvEbitda" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="9.20"></td>
                            </tr>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">Özsermaye Kârlılığı (ROE %)</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mSecRoe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="30.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1Roe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="35.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2Roe" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="25.00"></td>
                            </tr>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">Net Borç / FAVÖK</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mSecNetDebt" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="1.50"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1NetDebt" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="1.10"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2NetDebt" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="1.90"></td>
                            </tr>
                            <tr>
                                <td style="padding:10px; color:var(--text-200); font-weight:600;">Fiyat (₺)</td>
                                <td style="padding:6px 10px; text-align:center; color:var(--text-400); font-style:italic;">—</td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT1Price" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="100.00"></td>
                                <td style="padding:6px 10px;"><input type="number" step="0.01" id="mT2Price" class="sb-select" style="text-align:center; width:100%; box-sizing:border-box;" value="150.00"></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div style="display:flex; flex-direction:column; gap:8px; margin-top:16px;">
                    ...
                    <div style="display:flex; gap:16px;">
                        <label style="display:flex; align-items:center; gap:6px; font-family:var(--font-m); font-size:0.75rem; color:var(--text-200); cursor:pointer;">
                            <input type="radio" name="modalSaveLocation" value="private" checked style="accent-color:var(--blue); cursor:pointer;">
                            👤 Kişisel Sektör (Özel Bulut)
                        </label>
                        <label style="display:flex; align-items:center; gap:6px; font-family:var(--font-m); font-size:0.75rem; color:var(--text-200); cursor:pointer;">
                            <input type="radio" name="modalSaveLocation" value="shared" style="accent-color:var(--cyan); cursor:pointer;">
            ...
            <div style="padding:14px 20px; border-top:1px solid var(--border); display:flex; align-items:center; justify-content:end; gap:10px; background:var(--bg-2);">
                ...
                <button id="modalSubmitBtn" class="sb-refresh" style="width:auto; padding:8px 20px; margin:0; font-size:0.75rem;">SEKTÖRÜ EKLE</button>
            </div>
        </div>
        <style>
    ...
    const modalThT1 = modal.querySelector('#modalThT1');
    const modalThT2 = modal.querySelector('#modalThT2');

    modalT1.addEventListener('input', () => {
        modalThT1.textContent = modalT1.value.trim().toUpperCase() || 'Hisse 1';
    });
    modalT2.addEventListener('input', () => {
        modalThT2.textContent = modalT2.value.trim().toUpperCase() || 'Hisse 2';
    });
    ...
    modal.querySelector('#modalSubmitBtn').addEventListener('click', () => {
        const label = modal.querySelector('#modalSecName').value.trim();
        ...
        const isDuplicate = Object.values(SECTORS).some(sec => sec.label.trim().toLowerCase() === label.toLowerCase());
        if (isDuplicate) {
            showNotif('Bu sektör zaten tanımlı.');
            return;
        }

        const secPe = parseFloat(modal.querySelector('#mSecPe').value) || 0;
        const secPb = parseFloat(modal.querySelector('#mSecPb').value) || 0;
        const secEvEbitda = parseFloat(modal.querySelector('#mSecEvEbitda').value) || 0;
        const secRoe = parseFloat(modal.querySelector('#mSecRoe').value) || 0;
        const secNetDebt = parseFloat(modal.querySelector('#mSecNetDebt').value) || 0;

        const t1Pe = parseFloat(modal.querySelector('#mT1Pe').value) || 0;
        const t1Pb = parseFloat(modal.querySelector('#mT1Pb').value) || 0;
        const t1EvEbitda = parseFloat(modal.querySelector('#mT1EvEbitda').value) || 0;
        const t1Roe = parseFloat(modal.querySelector('#mT1Roe').value) || 0;
        const t1NetDebt = parseFloat(modal.querySelector('#mT1NetDebt').value) || 0;
        const t1Price = parseFloat(modal.querySelector('#mT1Price').value) || 0;

        const t2Pe = parseFloat(modal.querySelector('#mT2Pe').value) || 0;
        const t2Pb = parseFloat(modal.querySelector('#mT2Pb').value) || 0;
        const t2EvEbitda = parseFloat(modal.querySelector('#mT2EvEbitda').value) || 0;
        const t2Roe = parseFloat(modal.querySelector('#mT2Roe').value) || 0;
        const t2NetDebt = parseFloat(modal.querySelector('#mT2NetDebt').value) || 0;
        const t2Price = parseFloat(modal.querySelector('#mT2Price').value) || 0;
        ...
        const saveLocation = modal.querySelector('input[name="modalSaveLocation"]:checked').value;
        ...
        if (saveLocation === 'shared') {
            if (isLocalMode) {
                ...
                localShared.push(newSector);
                localStorage.setItem('bist_local_shared_sectors', JSON.stringify(localShared));
                window.dispatchEvent(new Event('localSharedSectorsUpdated'));
                
                showNotif(`✅ "${label}" ortak sektörü başarıyla paylaşıldı (Yerel Mod)!`, true);
            ...
            if (!db) {
                showNotif('Firebase bağlantısı hazır değil, ortak sektör paylaşılamadı.');
                return;
            }
            const sharedSectorsRef = window.collection(db, 'artifacts', appId, 'public', 'data', 'sectors');
            ...
            if (isLocalMode) {
                ...
                localPrivate.push(newSector);
                localStorage.setItem(privateKey, JSON.stringify(localPrivate));
                window.dispatchEvent(new Event(`localPrivateSectorsUpdated_${auth.currentUser.uid}`));
                
                showNotif(`✅ "${label}" kişisel sektörü kaydedildi (Yerel Mod)!`, true);
