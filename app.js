/* ═══════════════════════════════════════════════════════
   ...
   June 8, 2026 · BIST 100 ~13,885.19
...
//  BIST 100 ~13,885.19 · 15-min delayed yfinance feed
// ═══════════════════════════════════════════════════════
...
const ALL_STOCKS = {
    ...
    THYAO: {
        ...
        price: 298.00, change: 4.10, pct: 1.40,
    ...
    AKBNK: {
        ...
        price: 66.95, change: 1.25, pct: 1.90,
    ...
    TUPRS: {
        ...
        price: 238.60, change: 3.60, pct: 1.53,
    ...
    EREGL: {
        ...
        price: 38.96, change: 0.75, pct: 1.96,
    ...
    // ══ ENERGY SECTOR (CUSTOM MOCKS) ══
    EUPWR: {
        ticker: 'EUPWR', full: 'EUPWR.IS', sector: 'energy',
        name: 'Europower Enerji A.Ş.', short: 'Europower',
        price: 87.20, change: 2.20, pct: 2.59,
        pe: 12.50, pb: 3.20, evEbitda: 9.80, roe: 28.0,
        eps: 6.98, epsGrowth: 15.0, marketCap: 19e9,
        netDebtEbitda: 1.10, dividendYield: 0.5,
        color: '#4CAF50', colorDim: 'rgba(76,175,80,0.15)',
        isFavorite: true, discount: 42.1,
    },
    ALFAS: {
        ticker: 'ALFAS', full: 'ALFAS.IS', sector: 'energy',
        name: 'Alfa Solar Enerji A.Ş.', short: 'Alfa Solar',
        price: 61.15, change: 1.15, pct: 1.92,
        pe: 14.20, pb: 4.10, evEbitda: 11.50, roe: 32.0,
        eps: 4.30, epsGrowth: 18.0, marketCap: 22e9,
        netDebtEbitda: 0.90, dividendYield: 1.0,
        color: '#FFEB3B', colorDim: 'rgba(255,235,59,0.15)',
    },
};
...
const SECTORS = {
    ...
    energy: {
        ...
        icon: '⚡',
        tickers: ['EUPWR', 'ALFAS'],
        avg: { pe: 13.50, pb: 3.65, evEbitda: 10.60, roe: 30.0, netDebtEbitda: 1.00 },
    },
};
...
async function renderPerfChart() {
    ...
    try {
        ...
        const pxIdx = genPrices(BIST.index || 13885.19, numDays);
...
async function fetchLiveData(signal = null) {
    ...
    if (BIST.index === 0) { BIST.index = 13885.19; BIST.change = 185.19; BIST.pct = 1.35; }
...
function mergeAndRenderSectors() {
    ...
    // Track seen labels for name-based deduplication (case-insensitive & trimmed)
    const seenLabels = new Set();
    Object.values(SECTORS).forEach(sec => {
        if (sec.label) {
            seenLabels.add(sec.label.trim().toLocaleLowerCase('tr-TR'));
        }
    });

    // Helper to process custom sector safely with deduplication
    const loadCustomSectorData = (cSec, extraProps = {}) => {
        if (!cSec || !cSec.sectorData || !cSec.sectorData.label) return;
        const normalizedLabel = cSec.sectorData.label.trim().toLocaleLowerCase('tr-TR');
        if (seenLabels.has(normalizedLabel)) {
            // Already loaded a sector with this name, skip duplicate to prevent double rendering
            return;
        }
        seenLabels.add(normalizedLabel);
        
        SECTORS[cSec.key] = { ...cSec.sectorData, id: cSec.id, ...extraProps };
        if (cSec.stocksData) {
...
function deleteSectorFromDB(sectorKey, sectorLabel = null) {
    const normLabel = sectorLabel ? sectorLabel.trim().toLocaleLowerCase('tr-TR') : null;
    const filterItem = (item) => {
        if (item.key === sectorKey) return false;
        if (normLabel && item.sectorData && item.sectorData.label && item.sectorData.label.trim().toLocaleLowerCase('tr-TR') === normLabel) {
            return false;
        }
        return true;
    };
    ...
    if (isLocalMode) {
        ...
        localShared = localShared.filter(filterItem);
        ...
        if (auth && auth.currentUser) {
            ...
            localPrivate = localPrivate.filter(filterItem);
        ...
        const sharedSec = firebaseSharedSectors.find(item => item.key === sectorKey || (normLabel && item.sectorData && item.sectorData.label && item.sectorData.label.trim().toLocaleLowerCase('tr-TR') === normLabel));
        ...
        const privateSec = firebasePrivateSectors.find(item => item.key === sectorKey || (normLabel && item.sectorData && item.sectorData.label && item.sectorData.label.trim().toLocaleLowerCase('tr-TR') === normLabel));
    ...
    if (saved) {
        try {
            ...
            customSectorsList = customSectorsList.filter(filterItem);
...
function deleteSector(sectorKey) {
    ...
    deleteSectorFromDB(sectorKey, sec.label);
...
function showAddSectorModal(editSectorKey = null) {
    if (editSectorKey && typeof editSectorKey !== 'string') {
        editSectorKey = null;
    }
    ...
    modal.querySelector('#modalSubmitBtn').addEventListener('click', () => {
        ...
        const isDuplicate = Object.values(SECTORS).some(sec => {
            ...
            return sec.label.trim().toLocaleLowerCase('tr-TR') === label.trim().toLocaleLowerCase('tr-TR');
        });
        if (isDuplicate) {
        ...
        if (saveLocation === 'shared') {
            if (isLocalMode) {
                ...
                localShared = localShared.filter(item => {
                    if (editSectorKey && item.key === editSectorKey) return true;
                    if (item.key === key) return false;
                    if (item.sectorData && item.sectorData.label && item.sectorData.label.trim().toLocaleLowerCase('tr-TR') === normalizedLabel) {
                        return false;
                    }
                    return true;
                });
                ...
                Object.keys(SECTORS).forEach(k => {
                    if (SECTORS[k] && SECTORS[k].label && SECTORS[k].label.trim().toLocaleLowerCase('tr-TR') === normalizedLabel) {
                        delete SECTORS[k];
                    }
                });
            ...
            if (isLocalMode) {
                ...
                const normalizedLabel = label.trim().toLocaleLowerCase('tr-TR');
                localPrivate = localPrivate.filter(item => {
                    if (editSectorKey && item.key === editSectorKey) return true;
                    if (item.key === key) return false;
                    if (item.sectorData && item.sectorData.label && item.sectorData.label.trim().toLocaleLowerCase('tr-TR') === normalizedLabel) {
                        return false;
